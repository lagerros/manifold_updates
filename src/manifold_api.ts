...
export const getMarkets = async (): Promise<FetchedMarket[] | undefined> => {
  try {
    const response = await axios.get(
      "https://manifold.markets/api/v0/markets?limit=10"
    );
    return response.data;
  } catch (error) {
...
export const getComments = async (
  ...
  try {
    const response = await axios.get(
      `https://manifold.markets/api/v0/comments?contractId=${marketId}`
    );
    const allComments: Comment[] = response.data;

    const recentComments = allComments.filter(
      (comment) => moment().diff(moment(comment.createdTime), "hours") <= t
    );
...
export const getBets = async (
  ...
  try {
    const response = await axios.get(
      `https://manifold.markets/api/v0/bets?contractId=${marketId}`
    );
...
export const getUniquePositions = async (
  marketId: string
): Promise<
  { yesPositions: Position[]; noPositions: Position[] } | undefined
> => {
  try {
    const response = await axios.get(
      `https://manifold.markets/api/v0/bets?contractId=${marketId}`
    ...
    const uniquePositions = data.reduce(
...
export const getAggregateMoveData = async (
  marketId: string,
  t: number
): Promise<{ move: AggregateMove; counterMove: AggregateMove } | undefined> => {
  ...
  const getResponsibleShare = (mover: Mover, total: number) =>
    total !== 0 ? mover.probChangeTotal / total : 0;
  movers.forEach(getResponsibleShare);
  counterMovers.forEach(getResponsibleShare);

  const byProbChange = (a: Mover, b: Mover) =>
    Math.abs(b.probChangeTotal) - Math.abs(a.probChangeTotal);
    ...
    move: {
      ...
      stats: {
        ...
        top3moversEffect:
          movers
            .slice(0, 3)
            .reduce((sum, mover) => sum + mover.probChangeTotal, 0) / totalMove,
      },
    },
    counterMove: {
      ...
      stats: {
        ...
        effect20cohort: getPercentileEffect(
          0.2,
          counterMovers,
          totalCounterMove
        ),
        effect50cohort: getPercentileEffect(
          0.5,
          counterMovers,
          totalCounterMove
        ),
        effect80cohort: getPercentileEffect(
          0.8,
          counterMovers,
          totalCounterMove
        ),
        top3moversEffect:
          counterMovers
            .slice(0, 3)
            .reduce((sum, mover) => sum + mover.probChangeTotal, 0) /
          totalCounterMove,
      },
    },
  };
...
export const fetchCorrespondingMarkets = async (
  localMarkets: LocalMarket[]
): Promise<{ fetchedMarket: FetchedMarket; localMarket: LocalMarket }[]> => {
  ...
  return markets.filter(
    ({ fetchedMarket }): boolean => fetchedMarket !== undefined
  ) as { fetchedMarket: FetchedMarket; localMarket: LocalMarket }[];
};
